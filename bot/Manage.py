...
class Manage(commands.Cog):
    ...
    async def 타임아웃(self, ctx, user:discord.Member, min:int):
        if not ctx.author.guild_permissions.timeout_members:
            return await ctx.reply(embed=embed("타임아웃 실패","필수 요구사항인 **멤버 타임아웃**권한이 없습니다",discord.Color.red()))
    ...
    async def 타임아웃해제(self, ctx, user:discord.Member):
        if not ctx.author.guild_permissions.timeout_members:
            return await ctx.reply(embed=embed("타임아웃 해제 실패","필수 요구사항인 **멤버 타임아웃**권한이 없습니다",discord.Color.red()))
